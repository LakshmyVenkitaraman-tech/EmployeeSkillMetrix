...
<SkillsTable refreshTrigger={refreshKey} showTopRatedOnly={false} enableSearch={true} />  
  ...
  <button onClick={() => setShowForm(false)} className="text-gray-700 text-xl font-bold w-8 h-8 flex items-center justify-center rounded-full "> 
                ×  </button>
            </div>
<SkillForm onSaved={() => { setShowForm(false); 
                  handleRefresh();
              }} 
            />
          </div>
