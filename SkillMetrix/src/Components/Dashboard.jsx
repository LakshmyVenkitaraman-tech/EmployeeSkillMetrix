  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        let skillsList = [];
        try {
          skillsList = await api.getUserSkills(USER_ID);
        } catch (e) {
        ...
        let employmentList = [];
        try {
          employmentList = await api.getUserEmploymentHistory(USER_ID);
        } catch (e) {
...
<SkillsTable  refreshTrigger={refreshKey} showTopRatedOnly={true} enableSearch={false} />
        {skills.length === 0 && (
          <div className="text-gray-500 text-center py-4">
            No skills added yet. Add your first skill to see it here!
          </div>
        )}
      </div>
    </div>
